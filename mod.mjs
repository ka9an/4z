...
function parse(filter) {
    return Array.isArray(filter) ? filter.map((q1) => q1.split(":")) : [
        filter.split(":")
    ];
}
...
class Composer {
    ...
    chatType(chatType, ...middleware) {
        ...
        return this.filter((ctx) => ctx.chat?.type !== undefined && set.has(ctx.chat.type), ...middleware);
    }
    ...
    for (let i1 = 0, len = path3.length; i1 <= len; ++i1) {
        ...
        if (isPathSeparator1(code)) {
            if (lastSlash === i1 - 1 || dots === 1) { }
                ...
                if (res.length > 0)
                    res += separator + path3.slice(lastSlash + 1, i1);
                else
                    res = path3.slice(lastSlash + 1, i1);
...
function encodeWhitespace(string) {
    return string.replaceAll(/[\s]/g, (c1) => {
        return WHITESPACE_ENCODINGS[c1] ?? c1;
    });
}
    ...
    if (len > 1) {
        if (isPathSeparator(code)) {
            ...
            if (isPathSeparator(path9.charCodeAt(1))) {
                ...
                let last = j;
                for (; j < len; ++j) {
                    if (isPathSeparator(path9.charCodeAt(j)))
                        break;
                }
                if (j < len && j !== last) {
                    last = j;
...
function extname(path11) {
    ...
    for (let i7 = path11.length - 1; i7 >= start; --i7) {
        const code = path11.charCodeAt(i7);
        if (isPathSeparator(code)) {
            if (!matchedSlash) {
                startPart = i7 + 1;
                break;
...
function parse1(path12) {
    ...
    if (len > 1) {
        if (isPathSeparator(code)) {
            ...
            if (isPathSeparator(path12.charCodeAt(1))) {
                ...
                let last = j;
                for (; j < len; ++j) {
                    if (isPathSeparator(path12.charCodeAt(j)))
                        break;
                }
                if (j < len && j !== last) {
                    last = j;
    ...
    if (rootEnd > 0)
        ret.root = path12.slice(0, rootEnd);
    let startDot = -1;
...
function normalize1(path16) {
    ...
    if (path16.length === 0 && !isAbsolute1)
        path16 = ".";
    if (path16.length > 0 && trailingSeparator)
        path16 += "/";
    if (isAbsolute1)
        return `/${path16}`;
    return path16;
...
function basename1(path21, ext = "") {
    ...
    if (ext !== undefined && ext.length > 0 && ext.length <= path21.length) {
        if (ext.length === path21.length && ext === path21)
            return "";
        let extIdx = ext.length - 1;
        let firstNonSlashEnd = -1;
        for (i13 = path21.length - 1; i13 >= 0; --i13) {
            const code = path21.charCodeAt(i13);
            if (code === 47) {
                if (!matchedSlash) {
                    start = i13 + 1;
                    break;
...
function extname1(path22) {
    ...
    for (let i14 = path22.length - 1; i14 >= 0; --i14) {
        const code = path22.charCodeAt(i14);
        if (code === 47) {
            if (!matchedSlash) {
                startPart = i14 + 1;
                break;
...
function parse2(path23) {
    ...
    let preDotState = 0;
    for (; i15 >= start; --i15) {
        const code = path23.charCodeAt(i15);
        if (code === 47) {
            if (!matchedSlash) {
                startPart = i15 + 1;
                break;
...
class BufReader {
    ...
    #w = 0;
    #eof = false;
    static create(r2, size = 4096) {
        return r2 instanceof BufReader ? r2 : new BufReader(r2, size);
    }
    ...
    async readSlice(delim) {
        ...
        let slice;
        while (true) {
            let i17 = this.#buf.subarray(this.#r + s6, this.#w).indexOf(delim);
            if (i17 >= 0) {
                i17 += s6;
...
async function* iterateReader(r4, options) {
    const bufSize = options?.bufSize ?? DEFAULT_BUFFER_SIZE;
    const b1 = new Uint8Array(bufSize);
    while (true) {
        const result = await r4.read(b1);
        if (result === null) {
            break;
        }
...
function createCommonjsModule(fn, basedir, module) {
    return module = {
        path: basedir,
        exports: {},
        require: function (path25, base) {
            return commonjsRequire(path25, base === void 0 || base === null ? module.path : base);
        }
...
function setup(env) {
    ...
    function selectColor(namespace) {
        let hash = 0;
        for (let i19 = 0; i19 < namespace.length; i19++) {
            hash = (hash << 5) - hash + namespace.charCodeAt(i19);
            hash |= 0;
        }
    ...
    function enable(namespaces) {
        ...
        for (i20 = 0; i20 < len; i20++) {
            ...
            namespaces = split[i20].replace(/\*/g, ".*?");
            if (namespaces[0] === "-") {
    ...
    function enabled(name) {
        ...
        let i21;
        let len;
        for (i21 = 0, len = createDebug.skips.length; i21 < len; i21++) {
            if (createDebug.skips[i21].test(name)) {
                return false;
            }
        }
        for (i21 = 0, len = createDebug.names.length; i21 < len; i21++) {
            if (createDebug.names[i21].test(name)) {
                return true;
            }
        }
        return false;
...
var s1 = Object.getPrototypeOf, i = Object.prototype.hasOwnProperty;
var c = (e, t) => () => (t || e((t = {
    exports: {}
}).exports, t), t.exports);
var n = (e, t, o, d1) => {
    if (t && typeof t == "object" || typeof t == "function")
        for (let f of p(t))
            !i.call(e, f) && f !== o && m1(e, f, {
                get: () => t[f],
                enumerable: !(d1 = $(t, f)) || d1.enumerable
            });
    return e;
};
var a = (e, t, o) => (o = e != null ? u(s1(e)) : {}, n(t || !e || !e.__esModule ? m1(o, "default", {
    value: e,
    enumerable: !0
}) : o, e));
var r = c(() => { });
var l = a(r()), x = a(r()), { default: b, ...g } = x, q = (l.default ?? b) ?? g;
...
function session(options = {}) {
    ...
    const storage = options.storage ?? (debug3("Storing session data in memory, all data will be lost when the bot restarts."), new MemorySessionStorage());
    return async (ctx, next) => {
...
function undef(op, getSessionKey, opts = {}) {
    ...
    const reason = getSessionKey === defaultGetSessionKey ? "this update does not belong to a chat, so the session key is undefined" : "the custom `getSessionKey` function returned undefined for this update";
...
function webhookCallback(bot, adapter = defaultAdapter, onTimeout = "throw", timeoutMilliseconds = 10_000) {
    ...
    return async (...args) => {
        ...
        await timeoutIfNecessary(bot.handleUpdate(await update, webhookReplyEnvelope), typeof onTimeout === "function" ? () => onTimeout(...args)
